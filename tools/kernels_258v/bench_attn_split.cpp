// Unit: split-T decode attention vs legacy single-launch.
// Reports bitwise mismatch rate + max abs diff (functional parity, not
// bit-exact: the merge reorders FP ops) and rough kernel timing per (T, S).
...
int main(int argc, char **argv) {
  const char *spv_path =
      (argc > 1) ? argv[1] : "tools/kernels_258v/all_kernels.spv";
  ...
  CK(zeMemAllocDevice(ctx, &dd, (size_t)NQ * SMAX * 258 * 4, 64, dev,
                      (void **)&d_part));
  CK(zeMemAllocDevice(ctx, &dd, (size_t)NKV * MAXC * HD * 2, 64, dev,
                      (void **)&d_kc));
  CK(zeMemAllocDevice(ctx, &dd, (size_t)NKV * MAXC * HD * 2, 64, dev,
                      (void **)&d_vc));
  ...
  CK(zeCommandListAppendMemoryCopy(cp, d_gate, hg.data(), hg.size() * 4,
                                   nullptr, 0, nullptr));
  CK(zeCommandListAppendMemoryCopy(cp, d_kc, hkc.data(), hkc.size() * 2,
                                   nullptr, 0, nullptr));
  CK(zeCommandListAppendMemoryCopy(cp, d_vc, hvc.data(), hvc.size() * 2,
                                   nullptr, 0, nullptr));
  ...
  const int TS[] = {128, 512, 1024, 2048, 4096, 6720};
  const int SS[] = {2, 4, 8};
  std::printf("%-8s %-4s | %-14s %-14s %-10s | %-12s %-12s\n", "T", "S",
              "mismatch", "worst-abs", "bit?", "legacy-us", "split-us");
  for (int T : TS) {
    d_ctrl[1] = T - 1;
    // legacy reference
    ...
    CK(zeKernelSetArgumentValue(k_leg, 5, sizeof(void *), &d_ctrl));
    CK(zeKernelSetArgumentValue(k_leg, 6, sizeof(uint32_t), &maxc));
    ...
    for (int S : SS) {
      ...
      CK(zeKernelSetArgumentValue(k_sp, 4, sizeof(void *), &d_ctrl));
      CK(zeKernelSetArgumentValue(k_sp, 5, sizeof(uint32_t), &maxc));
      CK(zeKernelSetArgumentValue(k_sp, 6, sizeof(int), &S));
      ...
      std::printf("%-8d %-4d | %-14zu %-14.3e %-10s | %-12.1f %-12.1f\n", T,
                  S, mm, w, mm == 0 ? "EXACT" : "approx",
                  leg_us, sp_us);
    }
  }
  return 0;
