// T5.3 attention port: record ONCE a closed regular L0 list launching the
// decode-exact GQA+gate core (AttnCore: 24Q/4KV, dim 256, TMAX=64) with fixed
// addresses, replay 20x over varying Q/gate AND varying active_length driven
// by the control block (T5.4 policy: explicit 16 B update, no arg mutation).
// Checks: tight-tolerance match vs order-mirrored host ref (softmax exp can
// drift last-ulp across ISAs) + bitwise replay determinism.
// Usage: attn_replay <attn.spv> [report.json]
...
// BF16 KV: host reference works on RNE-quantized values (same rounding the
// device KvAppend performs), so ref-vs-device agreement stays at float-tail
// tolerance instead of degrading to bf16-eps.
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::fprintf(stderr, "usage: attn_replay <attn.spv> [report.json]\n");
    return 2;
  }
  ...
  CHECK(zeMemAllocDevice(ctx, &mdesc, sizeof(DecodeControl), 4096, dev,
                         &dCtrl));
  CHECK(zeMemAllocDevice(ctx, &mdesc, (size_t)24 * TMAX * 4, 4096, dev,
                         &dWts));
  ...
  if (zeKernelCreate(mod, &kdesc, &ker) != ZE_RESULT_SUCCESS) {
    std::fprintf(stderr, "kernel _ZTS8AttnCore not found\n");
    return 1;
  }
  ...
  std::vector<float> hQ(NH * D), hG(NH * D), hAtt(NH * D), hAtt2(NH * D),
      hRef(NH * D), hKc(NKV * TMAX * D), hVc(NKV * TMAX * D);
  for (auto &v : hKc)
    v = rnd();
  for (auto &v : hVc)
    v = rnd();
  ...
  for (int it = 0; it < IT; ++it) {
    ...
    for (auto &v : hG)
      v = rnd();
    int T = 1 + (it * 37 + 5) % TMAX; // 1..TMAX, non-monotonic
    DecodeControl c{9000 + it, T - 1, T, -1};
    ...
    for (size_t j = 0; j < hRef.size(); ++j) {
      ...
      if (rel > 1e-5) {
        ok = false;
        std::fprintf(stderr, "ref mismatch it %d j %zu: got %g want %g (T=%d)\n",
                     it, j, hAtt[j], hRef[j], T);
        break;
      }
      uint32_t a, b;
  ...
  double mR = med(tRep);
  std::printf("attn-replay med %.2f us/iter worst-rel %.2e %s\n", mR / 1e3,
              worstRel, ok ? "ATTN-OK" : "MISMATCH");
  char js[512];
  std::snprintf(js, sizeof js,
                "{\"device\":\"B60\",\"kernel\":\"_ZTS8AttnCore\","
                "\"heads\":\"24Q/4KV\",\"dim\":256,\"tmax\":%d,\"iters\":%d,"
                "\"replay_us\":%.2f,\"worst_rel\":%.2e,\"ref_tol\":1e-5,"
                "\"bitwise_deterministic\":%s,\"attn_ok\":%s}",
