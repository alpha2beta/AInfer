...
// Usage: gemvm3_db_replay <chunkgemmdb.spv> [report.json]
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::fprintf(stderr, "usage: gemvm3_db_replay <chunkgemmdb.spv> [report]\n");
    return 2;
  }
  int M = 256, K = 5120, N = 17408;
  if (argc >= 5)
    M = std::atoi(argv[4]);
  if (argc >= 6)
    K = std::atoi(argv[5]);
  ...
  if (zeKernelCreate(mod, &kdesc, &ker) != ZE_RESULT_SUCCESS) {
    std::fprintf(stderr, "kernel _ZTS11ChunkGemmDB not found\n");
    return 1;
  }
  ...
  // Optional argv[3]: v1 chunkgemm.spv for bitwise DB-vs-v1 equivalence
  // (same inputs, same order of accumulation => must match exactly).
  ze_kernel_handle_t ker1 = nullptr;
  void *dC1 = nullptr;
  std::vector<uint8_t> spv1;
  ze_module_handle_t mod1 = nullptr;
  if (argc >= 4) {
    FILE *sf1 = std::fopen(argv[3], "rb");
    if (!sf1) {
      std::fprintf(stderr, "no v1 spv\n");
      return 2;
    }
    std::fseek(sf1, 0, SEEK_END);
    size_t spvN1 = std::ftell(sf1);
    std::fseek(sf1, 0, SEEK_SET);
    spv1.resize(spvN1);
    if (std::fread(spv1.data(), 1, spvN1, sf1) != spvN1)
      return 2;
    std::fclose(sf1);
    ze_module_desc_t md1 = {ZE_STRUCTURE_TYPE_MODULE_DESC,
                            nullptr,
                            ZE_MODULE_FORMAT_IL_SPIRV,
                            spvN1,
                            spv1.data(),
                            nullptr,
                            nullptr};
    CHECK(zeModuleCreate(ctx, dev, &md1, &mod1, nullptr));
    ze_kernel_desc_t kd1 = {ZE_STRUCTURE_TYPE_KERNEL_DESC, nullptr, 0,
                            "_ZTS9ChunkGemm"};
    if (zeKernelCreate(mod1, &kd1, &ker1) != ZE_RESULT_SUCCESS) {
      std::fprintf(stderr, "kernel _ZTS9ChunkGemm not found\n");
      return 1;
    }
    CHECK(zeKernelSetGroupSize(ker1, 16, 1, 1));
    CHECK(zeMemAllocDevice(ctx, &mdesc, (size_t)M * N * 4, 4096, dev, &dC1));
    CHECK(zeKernelSetArgumentValue(ker1, 0, sizeof(void *), &dA));
    CHECK(zeKernelSetArgumentValue(ker1, 1, sizeof(void *), &dP));
    CHECK(zeKernelSetArgumentValue(ker1, 2, sizeof(void *), &dS));
    CHECK(zeKernelSetArgumentValue(ker1, 3, sizeof(void *), &dC1));
    CHECK(zeKernelSetArgumentValue(ker1, 4, sizeof(int), &mA));
    CHECK(zeKernelSetArgumentValue(ker1, 5, sizeof(int), &kA));
    CHECK(zeKernelSetArgumentValue(ker1, 6, sizeof(int), &nA));
    CHECK(zeKernelSetArgumentValue(ker1, 7, (size_t)512 * 2, nullptr));
    CHECK(zeKernelSetArgumentValue(ker1, 8, (size_t)256 * 2, nullptr));
    CHECK(zeKernelSetArgumentValue(ker1, 9, (size_t)512 * 4, nullptr));
  }
  ze_command_list_handle_t reg1 = nullptr;
  if (ker1) {
    CHECK(zeCommandListCreate(ctx, dev, &ldesc, &reg1));
    ze_group_count_t gc1 = {(uint32_t)((M / 32) * (N / 16)), 1, 1};
    CHECK(zeCommandListAppendLaunchKernel(reg1, ker1, &gc1, nullptr, 0, nullptr));
    CHECK(zeCommandListAppendBarrier(reg1, nullptr, 0, nullptr));
    CHECK(zeCommandListClose(reg1));
  }
  long v1_mismatch = -1;
  double v1_worstabs = 0;
  std::vector<uint16_t> hA(M * K);
  std::vector<float> hC(M * N), hC2(M * N), hRef(M * N), hC1(M * N);
  ...
  for (int it = 0; it < IT; ++it) {
    ...
    CHECK(zeCommandListAppendMemoryCopy(up, hC2.data(), dC, hC2.size() * 4,
                                        nullptr, 0, nullptr));
    if (ker1 && it == 0) {
      // Same uploaded inputs through the v1 kernel: must match bitwise
      // (identical summation order by construction).
      CHECK(zeCommandQueueExecuteCommandLists(qq, 1, &reg1, fence));
      CHECK(zeFenceHostSynchronize(fence, UINT64_MAX));
      CHECK(zeFenceReset(fence));
      CHECK(zeCommandListAppendMemoryCopy(up, hC1.data(), dC1, hC1.size() * 4,
                                          nullptr, 0, nullptr));
      v1_mismatch = 0;
      for (size_t i = 0; i < hC.size(); ++i) {
        uint32_t a, b;
        __builtin_memcpy(&a, &hC[i], 4);
        __builtin_memcpy(&b, &hC1[i], 4);
        double d = std::fabs((double)hC[i] - (double)hC1[i]);
        if (d > v1_worstabs)
          v1_worstabs = d;
        if (a != b) {
          if (v1_mismatch < 5)
            std::printf("V1-DIFF i %zu: db=%a v1=%a\n", i, hC[i], hC1[i]);
          ++v1_mismatch;
        }
      }
      std::printf("v1-vs-db: mismatch=%ld/%zu worstabs=%.3g\n",
                  v1_mismatch, hC.size(), v1_worstabs);
      if (v1_mismatch != 0)
        ok = false;
    }
    double refmax = 0;
  ...
  std::snprintf(js, sizeof js,
                "{\"device\":\"B60\",\"kernel\":\"_ZTS11ChunkGemmDB\","
