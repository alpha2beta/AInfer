// T7.4 chunk production: FULL full-attention chunk on REAL weights. One
// recorded list over an M=32 chunk at prefix P0=64 (trunk layer 3 INT4 spans
// from .binfer): per-row norms + fp16 converts, chunk GEMMs (qkv/k/v/o/
// gate/up/down), per-row split + Q/K norms, ChunkRope + ChunkKvAppend (new
// kernels: M-parallel rope/append at global positions), ChunkAttn over the
// BF16 prefix+chunk cache, o_proj + residual + post-norm + MLP, all in-list.
// Prefix cache pre-filled directly (math under test is the causal scan, not
// the fill path — single-pos KvAppend already proven). Checks: final hidden
// vs host float ref with dequant on the fly (tol 1e-3: fp16 chunk path) +
// guard slots stay zero (append stride check) + strict bitwise
// reset-determinism.
// Usage: chunkattnreal_replay <model.binfer> <10 spv...> [report.json]
//   order: norm cvtf32f16 chunkgemm splitqk batchnorm chunkrope chunkkvappend
//          chunkattn silumul resaddf
...
int main(int argc, char **argv) {
  if (argc < 12) {
    std::fprintf(stderr, "usage: chunkattnreal_replay <model> <10 spv> "
                         "[report]\n");
    return 2;
  }
  const char *model = argv[1];
  const int H = 5120, QW = 12288, KVW = 1024, QN = 6144, I = 17408;
  const int M = 32, P0 = 64, TC = P0 + M;
  ...
  const char *entries[] = {"_ZTS8RMSNormW", "_ZTS9CvtF32F16", "_ZTS9ChunkGemm",
                           "_ZTS7SplitQK",  "_ZTS9BatchNorm", "_ZTS9ChunkRope",
                           "_ZTS13ChunkKvAppend", "_ZTS9ChunkAttn",
                           "_ZTS7SiluMul",   "_ZTS7ResAddF"};
  ze_kernel_handle_t kh[10] = {nullptr};
  ...
  enum K { NORM, CVT, GEMM, SPLIT, BNORM, CROPE, CKV, CATTN, SILU, RES };
  for (int i = 0; i < 10; ++i)
  ...
  // ChunkAttn over prefix + freshly appended chunk rows.
  setarg(kh[CATTN], 0, sizeof(void *), &dAtt);
  setarg(kh[CATTN], 1, sizeof(void *), &dQn);
  setarg(kh[CATTN], 2, sizeof(void *), &dMKc);
  setarg(kh[CATTN], 3, sizeof(void *), &dMVc);
  setarg(kh[CATTN], 4, sizeof(void *), &dGate);
  setarg(kh[CATTN], 5, sizeof(void *), &dCtrl);
  setarg(kh[CATTN], 6, sizeof(int), &tmax);
  setarg(kh[CATTN], 7, sizeof(void *), &dWts);
  launch(kh[CATTN], M * 24);
  ...
  double mR = med(tRep);
  std::printf("chunkattnreal med %.2f us/chunk worst-rel %.2e %s\n", mR / 1e3,
              worstRel, ok ? "CHUNKATTNREAL-OK" : "MISMATCH");
  char js[640];
  std::snprintf(js, sizeof js,
                "{\"device\":\"B60\",\"layer\":\"full-attn chunked (real "
                "weights, layer 3)\",\"chunk\":%d,\"prefix\":%d,\"runs\":2,"
                "\"replay_us\":%.2f,\"worst_rel\":%.2e,\"ref_tol\":1e-3,"
                "\"guards_zero\":%s,\"reset_deterministic\":%s,"
                "\"chunkattnreal_ok\":%s}",
