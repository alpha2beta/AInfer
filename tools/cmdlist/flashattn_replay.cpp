...
// KV cache. Tolerance gate worst-rel 1e-6 (online summation order differs
// from row-softmax at 1-ulp level — NOT bitwise by design). Two configs:
// exact tiles (M=32) + partial tail tile (M=20).
// Usage: flashattn_replay <chunkflashattn.spv> [report.json]
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::fprintf(stderr, "usage: flashattn_replay <chunkflashattn.spv> [report]\n");
    return 2;
  }
  ...
  // Configs: {M rows, W slots, P prefix}. Causal limit per row: t < P+m+1.
  const int cfgs[][3] = {{32, 1024, 512}, {20, 768, 384}};
  for (int ci = 0; ci < 2; ++ci) {
    ...
    for (size_t i = 0; i < hQ.size(); ++i)
      hQ[i] = f32_to_bf16(frnd(1.0f));
    for (size_t i = 0; i < hKc.size(); ++i) {
      hKc[i] = f32_to_bf16(frnd(1.0f));
      hVc[i] = f32_to_bf16(frnd(1.0f));
    }
    // Host fp32 reference: per (m,hh), online softmax over valid keys.
    auto bf = [&](uint16_t b) { return bf16_to_f32(b); };
    auto hf = [&](uint16_t b) {
      // our "fp16" test values are bf16-rounded; decode as fp32 directly
      return bf(b);
    };
    for (int m = 0; m < M; ++m) {
