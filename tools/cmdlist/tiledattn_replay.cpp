...
// list (tiledattn.spv, TMAXF=4096 sizing). SLM-shared single-pass proof:
// correctness vs the SAME full host ref as attnfar (algorithm is an
...
// Usage: tiledattn_replay <tiledattn.spv> [report.json]
...
int main(int argc, char **argv) {
  if (argc < 2) {
    std::fprintf(stderr, "usage: tiledattn_replay <tiledattn.spv> [report.json]\n");
    return 2;
  }
  ...
  // 4 groups x 6 WIs (NOT 24 groups: gid addresses the KV head).
  CHECK(zeKernelSetGroupSize(ker, 6, 1, 1));
  int tmaxArg = TMAXF;
  ...
  CHECK(zeKernelSetArgumentValue(ker, 7, (size_t)64 * 256 * 2, nullptr));
  CHECK(zeKernelSetArgumentValue(ker, 8, (size_t)64 * 256 * 2, nullptr));
  ...
  ze_group_count_t gc = {(uint32_t)NKV, 1, 1};
  ...
  for (int T : TS) {
    ...
    if (!ok)
      break;
    std::printf("tiledattn T=%d %.2f ms\n", T, us / 1e3);
  }
  std::printf("tiledattn worst-rel %.2e %s\n", worstRel,
              ok ? "TILEDATTN-OK" : "MISMATCH");
  char js[512];
  std::snprintf(js, sizeof js,
                "{\"device\":\"B60\",\"kernel\":\"_ZTS9TiledAttn\","
                "\"t_points\":[1024,2048,4096],\"tmax\":%d,"
                "\"us_at_4096\":%.1f,\"worst_rel\":%.2e,\"ref_tol\":1e-5,"
                "\"gbs_at_4096\":%.1f,\"tiledattn_ok\":%s}",
                TMAXF, us4096, worstRel,
                // useful KV bytes per layer-scan at T=4096, single layer
                (double)(2.0 * 4 * 4096 * 256 * 2) / (us4096 / 1e6) / 1e9,
                ok ? "true" : "false");
  if (argc > 2) {
