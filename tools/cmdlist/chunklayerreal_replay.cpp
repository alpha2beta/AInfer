// T7.4 chunk production: FULL linear-layer chunk orchestration on REAL
// weights. One recorded list composes chunk GEMMs (qkv/z/b/a/out/gate/up/down
// at M=32, trunk layer 0 INT4 spans from .binfer) with chunk SSM (persistent
// conv history + recurrent state) and per-row stateless stages (norms, split,
// l2, beta, gating, silu, residuals), replayed as a 2-chunk sequence with
// conv/SSM state carrying A->B. fp16 GEMM inputs via in-list converts (layer
// stream stays fp32). Checks: chunk outputs vs host float ref with dequant
// on the fly (tol 1e-3: fp16 path, no INT8 flips here) + cross-chunk
// continuity (B-after-A differs from B-alone) + full reset-rerun bitwise
// identical.
// Usage: chunklayerreal_replay <model.binfer> <12 spv modules...> [report.json]
...
int main(int argc, char **argv) {
  if (argc < 14) {
    std::fprintf(stderr, "usage: chunklayerreal_replay <model> <12 spv> "
                         "[report]\n");
    return 2;
  }
  ...
  // ---- real weights: trunk layer-0 spans from the .binfer file ----
  ...
  Mat mats[] = {
      {"model.language_model.layers.0.linear_attn.in_proj_qkv.weight", C, H,
       40},
      {"model.language_model.layers.0.linear_attn.in_proj_z.weight", V6, H,
       40},
      {"model.language_model.layers.0.linear_attn.in_proj_b.weight", NH, H,
       40},
      {"model.language_model.layers.0.linear_attn.in_proj_a.weight", NH, H,
       40},
      {"model.language_model.layers.0.linear_attn.out_proj.weight", H, V6,
       48},
      {"model.language_model.layers.0.mlp.gate_proj.weight", I, H, 40},
      {"model.language_model.layers.0.mlp.up_proj.weight", I, H, 40},
      {"model.language_model.layers.0.mlp.down_proj.weight", H, I, 136}};
  for (auto &mt : mats) {
  ...
  // NOTE: A_log/dt_bias/conv1d are "copy" scheme = stored BF16 (not fp32);
  // load via the BF16 path like the norms (device and host share the exact
  // converted floats).
  std::vector<float> hInN, hPostN, hNG, hAL, hDT, hCW;
  load_norm("model.language_model.layers.0.input_layernorm.weight", dInN, H,
            hInN);
  load_norm("model.language_model.layers.0.post_attention_layernorm.weight",
            dPostN, H, hPostN);
  load_norm("model.language_model.layers.0.linear_attn.norm.weight", dNG, 128,
            hNG);
  load_norm("model.language_model.layers.0.linear_attn.A_log", dAL, NH, hAL);
  load_norm("model.language_model.layers.0.linear_attn.dt_bias", dDT, NH, hDT);
  load_norm("model.language_model.layers.0.linear_attn.conv1d.weight", dCW,
            C * 4, hCW);
  ...
  std::printf("chunklayer med %.2f us/chunk worst-rel %.2e %s\n", mR / 1e3,
              worstRel, ok ? "CHUNKLAYERREAL-OK" : "MISMATCH");
  char js[640];
  std::snprintf(js, sizeof js,
                "{\"device\":\"B60\",\"layer\":\"linear-attn chunked "
                "(real weights, layer 0)\",\"chunk\":%d,\"chunks\":2,"
