...
- Follow-up (2026-09-23): full-preset build closed — new `AINFER_ENABLE_SYCL` option (default ON, B60 behavior unchanged) set OFF in the 258v preset; all 11 SYCL-only B60-era blocks guarded (`attn`, `bench`, `blkblock`×2, `blkexec`, `decode`, `esimd_check`, `gemv`×2, `norm`, `prefill`×2, `sample_t39`; host-only `validate_t71` kept; `cmdlist` already excluded via `AINFER_BUILD_CMDLIST=OFF`). Same pass fixed the next blocker: `decode_l0` + `kvfit` linked a hardcoded Ubuntu loader path missing on CachyOS — both now use the `AINFER_ZE_LOADER` cache var (Ubuntu default, 258v preset points at the sysroot loader, mirroring `l0probe`). `cmake --build --preset 258v` green 8/8 targets; `ctest --preset 258v` still 6/6. B60 `b60`/`host` presets unaffected (SYCL ON, Ubuntu loader default).
