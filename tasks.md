...
- Follow-up analysis (2026-09-21): `tools/mtp/report_mtp_acceptance_analysis.json` — position effect is real (early-step α=0.43 → late-step α=0.82) but warmup gating is NOT recommended (trace sim within ±2% of always-speculate; draft is only 11% of trunk so breakeven α≈0.28). Main gap is 5–9 ms/round integration overhead (20–30% tax; explains projected-1.44x vs realized-1.198x). Next: profile per-submit timestamps, then attack round overhead. Note: `report_accept.json` is B60/Qwen3.8-era and excluded from 258V conclusions; 258V runtime is BF16-KV only (KV8×MTP needs an INT8-KV port).
