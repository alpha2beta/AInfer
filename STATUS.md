...
|---|---|---|
| Multi-Token Prediction (MTP) | ✅ **Single-token draft committed & verified (T10.1)** ⚠️ **dual-token extension uncommitted** | Committed: 19 MTP weights in pinned `.binfer`, dedicated 2 MiB KV cache, 3.256 ms draft latency (10.9% of trunk), pooled alpha = 67.50%, projected speedup 1.44x, bit-exact determinism (`tools/mtp/report_mtp_258v.json`). The dual-token verification path (`int4_gemv_m2`, `--speculative` serving flag, `report_speculative_258v.json`, up to 51.36 tok/s) exists only in the uncommitted working tree as of 2026-09-20 and is entangled with an unresolved decode regression — see Gate M6 note below before relying on it. |
