...
> Single-source current state. Updated 2026-09-18 (Phase 7 Passed, Gate M6 Signed Off; 42/62 tasks done).
...
|---|---|---|
| Single-process in-memory runtime | `decode_258v` (unified executable: in-memory prefill $\to$ recorded decode loop) | ✅ **Operational (Phase 5/7).** Zero disk re-uploads, shared arena state, 34.34–34.88 tok/s sustained decode, chunked batched prefill up to 89.31 tok/s (78.92 tok/s standard) |
...
| **Gate M1** | Phase 1: CachyOS & Lunar Lake foundation | `[~]` In Progress (2/8) | L0 probe done (`report_258v.json`), toolchain cached, memory contention benchmark pending |
...
| **Gate M4** | Phase 5: Unified single-process runtime | ✅ **PASSED (2026-09-18)** | `report_phase5.json`: 17.99 GiB committed memory (14.01 GiB headroom on 32 GB RAM); 8-tok prefill 6.59 ms/tok; recorded command lists replayed with 34.88 tok/s decode; 128-byte control block; diagnostic cache CRC32 round-trip; 10 repeated generation runs bit-identical with 0 KB RSS memory growth |
...
| **Gate M6** | Phase 7: Performance characterization | ✅ **PASSED (2026-09-18, Updated 2026-09-19)** | `report_bench_t71.json`: load 9.09s, cold TTFT 547.0ms, warm TTFT 537.6ms, prefill 39.07 tok/s, sustained decode 34.88 tok/s (single-cmd profiling 35.81 tok/s), jitter p50=28.5ms p95=29.5ms; `report_context_sweep.json`: prefill & decode matrix 1..64K; `report_roofline.json`: 1294.22 MiB/tok active traffic, 47.33 GB/s achieved bandwidth (98.6% single-stream efficiency vs 48 GB/s bus, 34.7% vs 136.53 GB/s theoretical peak); `report_thermal_steady_state.json`: 5.58 min, 7200 tok, 47°C -> 74°C peak -> 55.8°C steady, 23.39 tok/s (97% retention); `report_llama_comparison.json`: 34.88 tok/s outperforms llama.cpp Vulkan (29.33 tok/s) by 1.19x (+18.9%) and CPU baseline (9.98 tok/s) by 3.49x |
