...
| **Gate M5** | Phase 6: Quality qualification | ✅ **PASSED (2026-09-18)** | `report_quality_eval.json`: 187/200 passed (93.5%); `report_teacher_forced.json`: >80% top-1 match on generation continuations vs unquantized BF16 Hugging Face forward pass; `report_divergence_analysis.json`: 13 divergences categorized as benign phrasing/mental math; `report_long_context.json`: 4K needle retrieval 6/6 passed (100.0%), 16K/32K/64K tiers qualified with >= 12.76 GB free RAM; `report_kv8_quality.json`: BF16 KV production default (<16K); KV8 qualified optional at ≥16K (`report_kv8_16k_scale.json`: 1.23× 16K prefill, 4/4 needle; 32K: 13.52 tok/s prefill, 4/4 needle; `report_mtp_kv8.json`: 160/160 MTP parity) |
...
- **Greedy Decode:** 9.4 → **12.27 tok/s** (**+30.5% / 1.31× speedup**); step latency is ~80% dominated by 10-layer full-attention KV scanning (~65 ms out of ~81 ms) rather than static weight bandwidth (peak memory bandwidth is 136.53 GB/s on LPDDR5X-8533). See `claim_correction.md`.
