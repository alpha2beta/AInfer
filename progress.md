...
- **2026-09-24 (P2 Sprint 2 — I3.1 MTP Dual-Token DPAS Optimization — DONE):**
  - **Empirical Architecture & Microbenchmark Shootout:** Evaluated pure-FP32 coalesced vector GEMV (`int4_gemv_m2`), Intel Xe2 hardware DPAS systolic GEMV (`int4_gemv_m2_dpas`, using `intel_sub_group_f16_f16_matrix_mad_k16`), and SLM-staged prefill GEMMs (`int4_gemm_prefill`) on Intel Arc 140V. At $B=2$ verification, the workload is strictly memory-bandwidth bound (85 GB/s LPDDR5X) with low arithmetic intensity ($\sim 4\text{ FLOP/Byte}$); SLM barriers introduce high latency overhead (~130–200 µs), whereas direct coalesced 16B vector loads achieve optimal latency in 28–82 µs. DPAS requires FP16 inputs (`short2`), truncating 13 mantissa bits which accumulates slight rounding drift across 40 layers, whereas pure-FP32 vector GEMV operates in full FP32, executes faster (34.0 ms vs 35.5 ms verify), and guarantees 100% bit-exact mathematical parity with greedy decode. Dual-token LM Head argmax evaluates the full 248,320 vocabulary in **2.99 ms** (84.76 GB/s LPDDR5X bandwidth saturation).
